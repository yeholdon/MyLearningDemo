 ...
 * 
 * 4. 在回声服务器的基础上将收到的字符串通过管道传给创建的另一个进程
 * 这个进程专门负责把字符串存入文件。
 */
...
int main(int argc, char *argv[]) {
    ...
    // 创建保存字符串的子进程前设置管道，以使子进程复制得到管道描述符
    pipe(fds);
    pid = fork();
    if (pid == 0)
    {
        FILE *fp = fopen("echmsg.txt", "wt");// t为以文本模式打开
        // 创建缓冲区存管道传来的文件
        char msgbuf[BUF_SIZE];
        // 一次存的太少了，累计存10次收到的再关闭文件
        for (int i = 0; i < 10; i++)
        {
            int len = read(fds[0], msgbuf, BUF_SIZE);
            fwrite((void *)msgbuf, 1, len, fp); // 每次以1个字节为单位，写len个字节
        }
        fclose(fp);
        return 0;
    }
